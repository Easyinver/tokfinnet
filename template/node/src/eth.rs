...
/// Tokfin DB backend type.
pub type TokfinBackend<B, C> = fc_db::Backend<B, C>;

pub fn db_config_dir(config: &Configuration) -> PathBuf {
	config.base_path.config_dir(config.chain_spec.id())
}

/// Available tokfin backend types.
...
pub struct EthConfiguration {
	...
	/// Sets the tokfin backend type (KeyValue or Sql)
	#[arg(long, value_enum, ignore_case = true, default_value_t = BackendType::default())]
	pub tokfin_backend_type: BackendType,

	// Sets the SQL backend's pool size.
	#[arg(long, default_value = "100")]
	pub tokfin_sql_backend_pool_size: u32,

	/// Sets the SQL backend's query timeout in number of VM ops.
	#[arg(long, default_value = "10000000")]
	pub tokfin_sql_backend_num_ops_timeout: u32,

	/// Sets the SQL backend's auxiliary thread limit.
	#[arg(long, default_value = "4")]
	pub tokfin_sql_backend_thread_count: u32,

	/// Sets the SQL backend's query timeout in number of VM ops.
	/// Default value is 200MB.
	#[arg(long, default_value = "209715200")]
	pub tokfin_sql_backend_cache_size: u64,
}

pub struct TokfinPartialComponents {
	pub filter_pool: Option<FilterPool>,
	pub fee_history_cache: FeeHistoryCache,
	pub fee_history_cache_limit: FeeHistoryCacheLimit,
}

pub fn new_tokfin_partial(
	config: &EthConfiguration,
) -> Result<TokfinPartialComponents, ServiceError> {
	Ok(TokfinPartialComponents {
...
pub async fn spawn_tokfin_tasks<B, RA, HF>(
	task_manager: &TaskManager,
	client: Arc<FullClient<B, RA, HF>>,
	backend: Arc<FullBackend<B>>,
	tokfin_backend: Arc<TokfinBackend<B, FullClient<B, RA, HF>>>,
	...
	HF: HostFunctions + 'static,
{
	...
	match &*tokfin_backend {
		fc_db::Backend::KeyValue(b) => {
			task_manager.spawn_essential_handle().spawn(
				"tokfin-mapping-sync-worker",
				Some("tokfin"),
	...
	// Spawn Tokfin EthFilterApi maintenance task.
	...
	// Spawn Tokfin FeeHistory cache maintenance task.
	task_manager.spawn_essential_handle().spawn(
		"tokfin-fee-history",
		Some("tokfin"),
